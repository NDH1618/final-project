...
function PagingAdmin({ page, totalPage, onPageClick }) {
    const handlePageClicked = (page) => {
        console.log('Page: ', page)
        onPageClick(page)
    }

    const PagingAdmin = (page, pagingBarSize, totalPage) => {
    ...
    return (
        <>
            {/* <h6>thu nghiem xem nao</h6> */}
            <nav aria-label="Paging">
                <ul className="pagination">
                    {
                        PagingAdmin(page, 5, totalPage).map(p => {
                            return (
                                <PagingItem onPageClick={handlePageClicked} key={p} page={p} isActive={page === p} />
                            )
                        })
                    }
                </ul>
